...
abstract class IterableExtensions {
	 ...
	 *   a, b
	 */
	def final static <T> Iterable<? extends Iterable<T>> split(Iterable<T> input, (T) => boolean delimPredicate) {
		input.fold(
	 ...
	 */
	@Pure def static <T> Set<T> closure(T node, (T) => Iterable<T> closureFunction) {
	...
	@Pure def static <T,U> Iterable<U> flatmap(Iterable<T> iterable, (T) => Iterable<U> mapFunction) {
		iterable.map[mapFunction.apply(it)].flatten
	}
