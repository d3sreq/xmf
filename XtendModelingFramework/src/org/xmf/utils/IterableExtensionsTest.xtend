...
import static extension org.xmf.utils.IterableExtensions.*
...
class IterableExtensionsTest {
	...
	@Test def void testTransitiveClosureLimits() {
		assertArrayEquals("empty closure is ok", #[] as int[], 0.closure[])

		try{ // cannot start from null
			assertNull(null.closure[])
			fail
	...
	@Test def void testTransitiveClosureOnDAG() {
		...
		assertArrayEquals(#[2,3,4,5,6], 1.closure[DAG.get(it)].sort)
		assertArrayEquals(#[4,5,6],     2.closure[DAG.get(it)].sort)
		assertArrayEquals(#[4,6],       3.closure[DAG.get(it)].sort)
	}
	...
	@Test def void testTransitiveClosureOnNODAG() {
		...
		assertArrayEquals(#[1,2,3,4,5,6], 1.closure[NODAG.get(it)].sort)
		assertArrayEquals(#[1,2,3,4,5,6], 3.closure[NODAG.get(it)].sort)
		assertArrayEquals(#[6],           5.closure[NODAG.get(it)].sort)
	}
}